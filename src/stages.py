...
def cards_not_taken(game):
    bot.edit_message_text(
        'Oyun bitdi! Oyunçular kartlarını çəkmədilər.',
        chat_id=game['chat'],
...
def set_order(game):
    ...
    message_id = bot.send_message(
        game['chat'],
        f'{role_titles["don"].capitalize()}, seçiminizi etməli və komandanızın atış qaydasını təyin etməlisiniz\nnBunu etmək üçün oyunçuların nömrələrini vurun və sonra "Seçimi bitir" düyməsini vurun.',
...
def get_order(game):
    ...
    bot.edit_message_text(
        f'{role_titles["don"].capitalize()} əmri yazdı. {role_titles["mafia"].capitalize()}, tapşırığınızla bir zərf alın!',
        chat_id=game['chat'],
        message_id=game['message_id'],
        reply_markup=keyboard
    )
...
def get_votes(game):
    names = [(0, 'Səs verməyin')] + [(i + 1, p['name']) for i, p in enumerate(game['players']) if p['alive']]
    return '\n'.join([
        f'{i}. {name}' + (
...
def last_words_criminal(game):
    ...
    bot.edit_message_text(
        f'Bir oyunçu xalq səsverməsi ilə həbs edildi {criminal+1} ({game["players"][criminal]["name"]}).'
        if criminal is not None else 'Şəhər cinayətkar seçmədi.',
        chat_id=game['chat'],
        message_id=game['message_id']
    )
...
def night(game):
    message_id = bot.send_message(
        game['chat'],
        f'Gecə gəlir. Şəhər yuxuya gedir. {role_titles["mafia"].capitalize()}, çəkməyə hazırlaşın...'
    ).message_id
...
def shooting_stage(game):
    ...
    bot.edit_message_text(
        f'{role_titles["mafia"].capitalize()} qurban seçir.\n' + format_roles(game),
        chat_id=game['chat'],
        message_id=game['message_id'],
        reply_markup=keyboard
    )
...
def don_stage(game):
    ...
    bot.edit_message_text(
        f'{role_titles["mafia"].capitalize()} yuxuya gedib. {role_titles["don"].capitalize()} yoxlayır.\n' + format_roles(game),
        chat_id=game['chat'],
        message_id=game['message_id'],
        reply_markup=keyboard
    )
...
def sheriff_stage(game):
    ...
    bot.edit_message_text(
        f'{role_titles["don"].capitalize()} yuxuya gedib. Uyan {role_titles["sheriff"]} don öz işini edir.\n{format_roles(game)}',
        chat_id=game['chat'],
        message_id=game['message_id'],
        reply_markup=keyboard
    )
...
def last_words_victim(game):
    ...
    bot.edit_message_text(
        f'Sabahınız xeyir şəhər!\nKədərli xəbər: Bu gecə bir oyunçu öldürüldü {victim+1} ({game["players"][victim]["name"]}).',
        chat_id=game['chat'],
        message_id=game['message_id']
    )
